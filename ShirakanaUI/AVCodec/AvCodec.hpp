...
public:
	...
	Wav codec(const std::wstring& path, int sr, bool sc = false)
	{
        ...
        const double sample_ratio = double(sr) / double(inSampleRate);
        ...
        if (nSample != static_cast<size_t>(RawWavLen))
        {
            ...
            if (sc)
                for (size_t i = 0; i < nSample; ++i)
                    DataChun[i] = isnan(yi[i]) ? 0i16 : static_cast<short>(yi[i] * sample_ratio);
            else
                for (size_t i = 0; i < nSample; ++i)
